...
public class Street extends AbstractEdge implements WalkableEdge {
    ...
    /**
     * Streets with bike lanes are safer -- about twice as safe as streets without. This is how long
     * the street would have to be without bike lanes, to kill as many people as it presently kills
     * with bike lanes, statistically speaking.
     */
    ...
    public boolean getWheelchairAccessible(double maxAllowedSlope) {
        if (Math.abs(maxSlope) > maxAllowedSlope) { 
            return false;
        }
        return wheelchairAccessible;
    }
    ...
    /**
     * Override slope calculation to always return 0. This is useful when the street is a flat
     * bridge over sloping terrain
     * 
     * @param slopeOverride
     *            the slopeOverride to set
     */
