...
/**
 * This fare service impl handles the cases that GTFS handles. For more interesting fare structures, like New York's MTA, you get to implement your
 * own FareService. See this thread on gtfs-changes explaining the proper interpretation of fares.txt:
...
public class DefaultFareServiceImpl implements FareService, Serializable {
    ...
    public Fare getCost(GraphPath path) {
        ...
        if (fareAttributes.size() > 0) {
            wrappedCurrency = new WrappedCurrency(fareAttributes.values().iterator().next()
                    .getCurrencyType());
        }
