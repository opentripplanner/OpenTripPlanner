...
/**
 * Filters nearby stops for transfer generation based on trip patterns and flex trips. Returns only
 * stops where boarding or alighting is possible, and for each pattern/trip, returns only the
 * closest stop to minimize transfer generation.
 */
interface NearbyStopFilter {
   ...
   * <p>
   * Note: The result will include the origin stop if it is a StopVertex. This is intentional -
   * we don't want to return the next stop down the line for patterns passing through the origin.
   *
