 ...
 * <p>
 * This class lives in the {@code transaction} package even though only
 * {@code maxSnapshotFrequency} is directly related to transaction concerns, while
 * {@code purgeExpiredData} belongs to the updater layer. The two parameters cannot be split into
 * separate configuration classes because doing so would break backwards compatibility of the
 * configuration format. Placing both parameters here is the only way to avoid circular dependencies
 * between the {@code transaction} and {@code updater} packages.
 */
