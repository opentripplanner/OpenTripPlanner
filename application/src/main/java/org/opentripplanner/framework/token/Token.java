 ...
 * fields. The responsibility of enforcing required fields is left to the concrete domain specific
 * representation, like the {@code PageCursor}.
 */
public class Token {
  ...
  /**
   * Be careful with enums. If values are added or deleted the backward/forward compatibility is
   * compromised. This method return an empty value if the enum does not exist.
   * <p>
   * To ensure that enum values are forward compatible the value must first be added, and then it
   * can not be used in a token before OTP is released and deployed. Then when the enum value exist
   * in the deployed server, then a new version of OTP can be rolled out which now can use the new
   * value.
   * <p>
   * To ensure backwards compatibility, enum values should be **deprecated**, not removed. The
   * enum value can only be deleted, when all tokens with the value has expired (depends on
   * use-case).
   */
