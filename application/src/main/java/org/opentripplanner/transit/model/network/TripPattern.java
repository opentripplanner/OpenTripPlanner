 ...
 * possible exceptions for TransitMode and SubMode). TODO RT_AB: We need to clarify exactly which
 * characteristics are identical across the trips. Grouping into patterns serves more than one
 * purpose: it conserves memory by not replicating details shared across all trips in the
 * TripPattern; it reflects business practices outside routing; it is essential to optimizations in
 * routing algorithms like Raptor. We may be conflating a domain model grouping with an internal
 * routing grouping. TODO RT_TG In addition to AB comment: this does not map cleanly to NeTEx
 * JourneyPattern, the concept does not exist in GTFS. The TripPattern ID is unfortunatly exposed in
 * the OTP APIs. This class has a 1-to-1 relationship with RoutingTripPattern. To support a more
 * flexible system and more use-cases we should consider keeping "routing-tables" apart from the
 * bussiness domain model. One example, if the wheelchair accessability differ between trips within
 * the same pattern, then we can not support it - wheelchair is not part of the TripPattern key.
  ...
  implements Cloneable, LogInfo
{
   ...
   * Javadoc on DefaultTimetableRepository for more details). TODO RT_AB: The above system should be
   * changed to integrate realtime and scheduled data more closely. The Timetable may become
   * obsolete or change significantly when they are integrated.
   */
   ...
   * have the same direction: - trips derived from NeTEx data belong to a ServiceJourney that
   * belongs to a JourneyPattern that belongs to a NeTEx Route that specifies a single direction. -
   ...
   * {@link org.opentripplanner.updater.trip.patterncache.TripPatternCache} See comment in
   * TripPatternCache.
   */
  @Deprecated
  @Nullable
