 ...
 * possible to load/access it in the cache and CPU for performance reasons. - Also, we deduplicate
 * these so a RTP can be reused by more than one TP. - This also provide explicit documentation on
 * which fields are used during a search and which are not.
 */
