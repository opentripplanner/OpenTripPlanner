...
/**
 * <p>
 * This is the ROOT mapper to map from the Netex domin model into the OTP internal model. This
 * class delegates to type/argegate specific mappers and take the result from each such mapper and
 * add the result to the {@link TransitDataImportBuilder}.</p>
 * <p>
 ...
 * direct references between entities in the OTP domain model.</p>
 */
public class NetexMapper {
   ...
   * indexes to netex entities, as well as global indexes to OTP domain objects needed in the
   * mapping process. Some of these indexes are feed scoped, and some are file group level scoped.
   * As a rule of thumb the indexes for OTP Model entities are global(small memory overhead), while
   * the indexes for the Netex entities follow the main index {@link #currentNetexIndex}, hence
   * sopped by file group.
   */
   ...
   * {@link NetexEntityIndexReadOnlyView}.</p>
   * <p>
   ...
   * a reference on Agency, and Notice may reference on Route.</p>
   *
