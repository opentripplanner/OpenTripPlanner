...
Runs GraphQL trip queries in a background thread during OTP startup to warm up the application (JIT
compilation, GraphQL schema caches, routing data structures, etc.) before production traffic
arrives.

## Lifecycle

1. `ConstructApplication` obtains a `WarmupLauncher` from the Dagger factory
   (`configure.WarmupModule` wires it) and calls `start()` after Raptor transit data is created and
   updaters are configured.
...
`WarmupParameters` (the configured parameter values) and `WarmupApi` (which GraphQL API to
exercise). `WarmupConfig` (in `standalone.config.routerconfig`) reads the JSON config section and
produces a `WarmupParameters` instance.
