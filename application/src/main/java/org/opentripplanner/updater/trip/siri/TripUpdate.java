 ...
 * @param serviceDate                the service date for which this update applies (updates are
 *                                   valid only for one service date)
 ...
 * @param addedTripPattern           optionally if this trip update adds a new trip pattern , the
 *                                   new trip pattern to this new trip.
 * @param routeCreation              true if an added trip cannot be registered under an existing
 *                                   route and a new route must be created.
 * @param dataSource                 the dataSource of the real-time update.
 * @param hideTripInScheduledPattern when non-null, signals the snapshot manager to mark the trip as
 *                                   deleted in this pattern.
 */
record TripUpdate(
