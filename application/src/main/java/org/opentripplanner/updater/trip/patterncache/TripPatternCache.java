 ...
 * the same sequence of stops, they will all end up on this same TripPattern. TODO RT_TG: There is
 * no clear strategy for what should be in the cache and the transit model and the flow between
 * them. With the increased usage of DatedServiceJourneys, this should probably be part of the main
 * model - not a separate cache. It is possible that this class works when it comes to the
 * thread-safety, but just by looking at a few lines of code I see problems - a strategy needs to be
 * analysed, designed and documented. TODO RT_VP TripPatternCache caches RT patterns keyed by
 * StopPattern only, setting originalTripPattern from the first trip that created the entry in the
 * cache. When a second trip on a different route produces the same modified StopPattern, the cache
 * returns a pattern with potentially the wrong originalTripPattern. If the updater uses
 ...
 * will get null → TRIP_NOT_FOUND_IN_PATTERN).
 *
 */
