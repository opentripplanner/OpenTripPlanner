 ...
 * <ul>
 *   <li>{@link #shouldBeFlaggedForRemoval()} - If filtering is done based on looking at one itinerary
 *       at the time.</li>
 *   <li>{@link #flagForRemoval(List)}}) - If you need more than one itinerary to decide which to
 *       delete.</li>
 * </ul>
 */
public interface RemoveItineraryFlagger {
   ...
   * list passed to {@link RemoveItineraryFlagger#flagForRemoval(List)}. The default value is true,
   * as usually the already removed itineraries are not needed further in the filter chain.
   */
  ...
  /**
   * Filter given {@code input} and remove itineraries which should be flagged for removal. This can
   * be used in unit-tests - either testing the filter or using the filter in a test.
   * <p>
