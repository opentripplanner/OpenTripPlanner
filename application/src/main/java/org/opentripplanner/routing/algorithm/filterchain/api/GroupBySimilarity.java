 ...
 * The itineraries are grouped by the legs that account for more then 'p' % for the total distance.
 * <p/>If the time-table-view is enabled, the result may contain similar itineraries where only the
 ...
 * are skipped. Then for each group of itineraries the itinerary with the lowest cost is kept. All
 * other itineraries are dropped.
 * <p>
 ...
 * </pre>
 * Normally, we want some variation, so a good value to use for this parameter is the
 * combined cost of board- and alight-cost including indirect cost from board- and alight-slack.
 *
