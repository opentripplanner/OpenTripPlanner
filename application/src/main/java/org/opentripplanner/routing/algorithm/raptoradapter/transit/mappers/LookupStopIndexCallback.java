...
/**
 * The raptor mapper does not have access to the transit layer, so it needs help to lookup
 * stop-location indexes (Stop index used by Raptor). There is a one-to-one mapping between stops
 * and stop-index, but a station, multimodal-station or group-of-stations will most likely contain
 * more than one stop.
 */
@FunctionalInterface
public interface LookupStopIndexCallback {
  /**
   * The implementation of this method should list all stop indexes part of the entity referenced by
   * the given id.
   *
