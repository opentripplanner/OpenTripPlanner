...
/**
 * <p>
 * In OTP, the street search and transit search are done separately. The results are then merged
 * and filtered to remove non-optimal itineraries. But, when the client does NOT provide a
 * ´directMode´, OTP does not do the streetSearch. And, the removal of non-optimal results is not
 * done, there are no street results to use to prune bad transit results with. In other words, OTP
 ...
 *   <li>Itinerary-2:
 *       {@code Origin ~ Walk 120m ~ Stop A ~ Bus 10:00 10:05 ~ Stop B ~ Walk 200m ~ Destination} (Raptor
 *       search)</li>
 * </ul>
 * <p>
 * <b>API level results</b>
 * <ul>
 *   <li>Request 1: {@code directMode:WALK, access/egressMode: WALK, transitModes:[TRANSIT] }, OTP
 *       returns [ Itinerary-1 ]</li>
 *   <li>Request 2: {@code directMode:null, access/egressMode: WALK, transitModes:[TRANSIT] }, OTP
 *       returns [ Itinerary-2 ]</li>
 * </ul>
 * <p>
 * In this case Itinerary-1 is clearly better than Itinerary-2. Some OTP clients run a separate
 * searches for WALK-ALL-THE_WAY, BIKE-ALL-THE-WAY, and WALK+TRANSIT. In this case, it would be best
 * for if Request 2 above return an empty set, not the silly Itinerary-2.
 * <p>
 * If no directMode is set and if the page cursor does not exist, the responsibility of this
 * class is to always filter away itineraries with a generalized-cost that is higher than the
 ...
 * <p>
 * If we compare itineraries by walking distance(not cost), we would still get silly results.
 * E.g. if the only stop reachable within walking distance is a train station, we could get a
 * itinerary like this:
 * <p>
 * {@code Origin ~ Walk 50m ~ Stop 1a ~ Train 10:00 10:15 ~ Stop B ~ Train 10:20 10:35 ~ Stop 1d ~
 * Walk 50m ~ Destination } NOT: {@code Origin ~ Walk 120m ~ Destination }
 */
public class FilterTransitWhenDirectModeIsEmpty {
  ...
  /**
   * If the direct mode was not set and if the page cursor does not exist, the WALK mode is returned
   * for filtering itineraries.
   */
  ...
  /**
   * This method determines whether to filter itineraries that only use walking. If the direct mode
   * was not set and if the page cursor does not exist, the filtering is performed. This is used
   * together with the resolveDirectMode method.
   */
