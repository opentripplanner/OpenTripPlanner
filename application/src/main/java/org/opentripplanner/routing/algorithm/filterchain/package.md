...
ItineraryListFilterChain is a mechanism for post-processing itinerary results. There are filters for
removing itineraries, sorting them and for decorating itineraries with more information like fares.
The filter chain is also responsible for creating routing errors, if no itineraries remain after
filtering. Itineraries are flagged for deletion, but not deleted when debugging is turned on. When
debugging is off, the chain removes those itineraries after all filters are complete.
