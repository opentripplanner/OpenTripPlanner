...
/**
 * The per-HTTP-request context threaded through the GTFS GraphQL API's data fetchers, bundling the
 * services a query may need (routing, transit, fares, realtime vehicles, ...).
 * <p>
...
public interface GtfsGraphQLRequestContext {
  ...
  /**
   * Returns a clone of the default route request. The clone is necessary because one HTTP request
   * can lead to several GraphQL queries, for example through batch or alias queries.
   */
