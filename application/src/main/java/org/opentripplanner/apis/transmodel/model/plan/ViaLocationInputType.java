...
public class ViaLocationInputType {
  ...
  private static final String DOC_LABEL = "The label/name of the location. This is pass-through " +
  ...
  static final GraphQLInputObjectType VISIT_VIA_LOCATION_INPUT = GraphQLInputObjectType
    ...
    .field(
      b -> b.name(FIELD_MINIMUM_WAIT_TIME)
    ...
    .field(
      b -> b.name(FIELD_STOP_LOCATION_IDS)
        .description(DOC_STOP_LOCATION_IDS)
        .type(optionalListOfNonNullStrings())
    )
    .field(
      b -> b.name(FIELD_COORDINATE).description(DOC_COORDINATE).type(CoordinateInputType.INPUT_TYPE)
    )
    .build();
