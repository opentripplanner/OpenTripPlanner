...
/**
 * Behavior configured for a single GBFS network, shared by the vehicle rental graph builder and the
 * vehicle rental service directory.
 * <p>
 * The network name is not part of this record: it is the key these parameters are looked up by
 * in {@link GbfsNetworkOverrides}, and the same type describes the {@code defaults} block, which
 * has no network of its own.
 *
 * @param geofencingZonePhase              which OTP phase computes and applies this network's
 *                                         geofencing zones. Configured as
 *                                         {@code applyGeofencingZones}.
 * @param requireDropOffInsideBusinessArea whether a rented vehicle must be dropped off before
 *                                         leaving the operator's business area, i.e. whether the
 *                                         router forces a drop-off at its border. Has no effect
 *                                         when {@code geofencingZonePhase} is
 *                                         {@link GeofencingZoneOtpPhase#OFF}.
...
public record GbfsNetworkParameters(
  GeofencingZoneOtpPhase geofencingZonePhase,
  ...
  public GbfsNetworkParameters {
    Objects.requireNonNull(geofencingZonePhase);
  }
}
