...
/**
 * Mutable repository for the realtime vehicles.
 */
public class DefaultRealtimeVehicleRepository implements RealtimeVehicleRepository {
  ...
  /** Create a repository initialized with the given state, see
   * {@link DefaultRealtimeVehicleRepositorySnapshot#copyOnWrite()}. */
  DefaultRealtimeVehicleRepository(Multimap<TripPattern, RealtimeVehicle> initialState) {
    this.vehicles = ArrayListMultimap.create(initialState);
  }
  ...
  /** Freeze the current state of this repository into an immutable snapshot. */
  DefaultRealtimeVehicleRepositorySnapshot freeze() {
    return new DefaultRealtimeVehicleRepositorySnapshot(ImmutableListMultimap.copyOf(vehicles));
  }
}
