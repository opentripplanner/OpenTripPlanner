 ...
 * <p>
 * The entire repository is serialized in the <em>graph.obj</em> file, so it needs to be
 * serializable.
 * <p>
 ...
 * be ok to drop this interface and let the service implementation also hold the data - serving both
 * the role of the Service interface and implementing the repository. The
 * world-envelope-graph-builder is the only component which needs the repository and making it more
 * tightly coupled would be ok. But, with the repository interface it is loosely coupled.
 */
