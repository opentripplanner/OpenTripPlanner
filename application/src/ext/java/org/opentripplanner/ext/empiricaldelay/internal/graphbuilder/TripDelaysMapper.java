...
class TripDelaysMapper {
  ...
  private TIntObjectHashMap<EmpiricalDelay> map(
    ...
    for (DelayAtStopDto delay : delayDtos) {
      ...
      if (i == stopIds.size()) {
        addStopMismatchIssue(tripId, serviceId, delay);
        return null;
      }
  ...
  private void addStopMismatchIssue(FeedScopedId tripId, String serviceId, DelayAtStopDto delay) {
    issueStore.add(
      "EmpiricalDelayStopMismatch",
