 ...
 * <p>
 * A trip is rejected when it is outside the passenger's window. Trips that pass are still
 * subject to expensive routing and a tighter post-filter on the actual itinerary times.
 *
 * <h2>Variables</h2>
 *
 ...
 *
 * The carpool window is {@code [trip.startTime, trip.endTime]} but the passenger boards and alights
 * somewhere in the middle. Two slacks pad the request window outward so trips that could still pick
 * up or drop off a passenger inside their window are not rejected:
 *
 * <ul>
 ...
 *       pickup/dropoff. Used wherever a single walk segment shifts the relevant bound by a known, bounded
 *       amount: a passenger leaving by LDT can be at the pickup as late as {@code LDT + W}; a carpool
 *       that drops off by {@code EAT − W} still gives an arrival at or after EAT after a W-long walk to
 *       destination.</li>
 *   <li><strong>{@code T} = {@link #MAX_TOTAL_TRAVEL_TIME}</strong> (max total passenger travel time,
 *       fallback) — used <em>only</em> in the two cells where neither the request window nor {@code W}
 *       can produce a real bound: <em>access / arriveBy=true / too early</em> and <em>egress /
 *       arriveBy=false / too late</em>. In both cases the carpool sits on the opposite end of the journey
 *       from the request anchor, separated from it by a transit ride of unknown duration. {@code T} caps
 *       that unknown duration with a deliberately conservative number so the filter degrades to "almost a
 *       no-op" in those cells rather than producing false negatives.</li>
 * </ul>
 *
 * <h2>Rules</h2>
 *
 ...
 *
 * <ul>
 *   <li><em>Too late</em> compares {@code tripStart} (earliest moment any pickup along the route can
 *       happen) against the latest the passenger could be at that pickup. {@code W} is added when the
 *       request anchors departure (arriveBy=false) and a walk pads the bound; {@code T} replaces
 *       {@code W} on the egress/false cell because access + transit between the anchor and the egress
 *       carpool is otherwise unbounded.</li>
 *   <li><em>Too early</em> compares {@code tripEnd} (latest moment any pickup or dropoff along the
 *       route can happen) against the earliest the passenger could be there. {@code W} is subtracted when
 *       a single dropoff walk separates the carpool from the destination anchor (direct & egress with
 *       arriveBy=true); {@code T} replaces {@code W} on the access/true cell because transit + egress
 *       between the carpool dropoff and the anchor is otherwise unbounded.</li>
 *   <li>The four cells with neither {@code W} nor {@code T} are the cases where the carpool is on the
 *       same side as the anchor and no walk or padding can move the bound.</li>
 * </ul>
 *
...
public class TimeTripFilter implements CarpoolTripFilter {
   ...
   *   <li>Too late: {@code tripStart > LDT + maxConnectionTime} where maxConnectionTime is {@code T}
   *       for egress (access + transit on the origin side is unbounded) and {@code W} otherwise.</li>
   * </ul>
   */
