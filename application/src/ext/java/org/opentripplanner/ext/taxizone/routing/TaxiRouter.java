...
/**
 * Handles taxi zone routing, filtering and decoration, backed by a single {@link TaxiZoneIndex}.
 * <ol>
 *   <li>Before a direct taxi street search runs, {@link #routeDirect} checks that the request
 *   origin and destination are covered by a common taxi zone provider; if not, an empty result
 ...
 *   by a common taxi zone provider, so RAPTOR never considers a combination that would later be
 *   rejected.
 ...
 *   TraverseMode#CAR} {@link StreetLeg} with a {@link TaxiZoneLeg} decorated with the matching
 *   zone, looked up using the same logical coordinates rather than the leg's own local
...
public class TaxiRouter {
   ...
   * TaxiZoneLeg}, looking up the covering zone via {@code pickup}/{@code dropoff} once (not per
  ...
  private List<Leg> decorateLegs(List<Leg> legs, WgsCoordinate pickup, WgsCoordinate dropoff) {
    ...
    if (taxiZone.isEmpty()) {
      LOG.warn("No taxi zone covers the pre-filtered taxi legs between {} and {}", pickup, dropoff);
      return legs;
    }
