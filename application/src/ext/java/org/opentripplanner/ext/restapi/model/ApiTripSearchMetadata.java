...
public class ApiTripSearchMetadata {
   ...
   * <p>
   * If the {@code numOfItineraries} request parameter is set, optimal itineraries are removed from
   * the end of the result. Be aware of this when adding the results of more than on search
   ...
   * need to find the trips that arrive immediately AFTER the latest-arrival-time, be sure NOT to
   * set the {@code numOfItineraries} request parameter in the next request.
   * <p>
   ...
   * need to find the trips that depart immediately BEFORE the earliest-departure-time, be sure NOT
   * to set the {@code numOfItineraries} request parameter in the next request.
   * <p>
   * If OTP for some reason is not able to calculate this value then it will be {@code null}.
   * <p>
   * Unit : epoch milliseconds
