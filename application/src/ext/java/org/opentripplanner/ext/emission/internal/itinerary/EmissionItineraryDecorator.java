...
public class EmissionItineraryDecorator implements ItineraryDecorator {
  ...
  public Itinerary decorate(Itinerary itinerary) {
    ...
    for (var l : itinerary.legs()) {
      ...
      } else if (l instanceof TaxiLeg taxiLeg) {
        value = calculateCo2EmissionsForTaxi(taxiLeg).orElse(null);
      } else {
        newLegs.add(l);
        continue;
      }
  ...
  private Optional<Emission> calculateCo2EmissionsForTaxi(TaxiLeg leg) {
