...
public class StreetModeLinkingTest extends GraphRoutingTest {
  ...
  /**
   * This is used to make parallel streets by adding the value to the longitude. The value make
   * streets about 11m apart.
   */
  ...
  /**
   * A place used as dummy to/from, when testing from/to. It can be anywhere, except the same
   * location as the place under test.
   */
  ...
  /**
   * Only CAR linking is handled specially, since walking with a bike is always a possibility, and
   * so no difference is made between BIKE/WALK:
   */
  @ParameterizedTest
  ...
  record LinkingTestCase(
    ...
    /**
     * Create a random place relativly close, but not on the street generated by this test-case. The
     * longitude for the place is the same as the street plus the {@link #OFFSET}. If this test-case
     * is street N, then the closest street for the location is in order:
