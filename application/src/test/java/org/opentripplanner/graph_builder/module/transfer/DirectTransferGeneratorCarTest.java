...
class DirectTransferGeneratorCarTest extends GraphRoutingTest {
  ...
  public void testRequestWithCarsAllowedPatternsWithDurationLimit() {
    var transferParameters = new TransferParametersForMode.Builder().withCarsAllowedStopMaxDuration(
      Duration.ofSeconds(10)
    ).withDisableDefaultTransfers(true).build();

    var repository = testDataWithStreetFraphAndPatterns().withCarFerrys_FARAWAY_S0_S12_and_S22_S23()
      .withTransferRequests(REQUEST_WITH_CAR_TRANSFER)
      .addTransferParameters(StreetMode.CAR, transferParameters)
      .build();
  ...
  public void testMaxTransferDurationForMode() {
    var transferParametersBuilderWalk = new TransferParametersForMode.Builder().withMaxDuration(
      Duration.ofSeconds(100)
    ).build();
    var transferParametersBuilderBike = new TransferParametersForMode.Builder().withMaxDuration(
