...
public class ServiceDateTest {
  ...
  public void toZonedDateTime() {
    ...
    assertEquals(
      "2020-08-23T23:00+02:00[Europe/Oslo]",
      d.toZonedDateTime(zone, -25 * 3600).toString()
    );
    assertEquals(
      "2020-08-26T12:00+02:00[Europe/Oslo]",
      d.toZonedDateTime(zone, 36 * 3600).toString()
    );

    // Time is adjusted 1 hour back in Norway on this date
    d = new ServiceDate(2020, 10, 25);
    assertEquals(
      "2020-10-24T23:59:59+02:00[Europe/Oslo]",
      d.toZonedDateTime(zone, -3601).toString()
    );

    // Time is adjusted 1 hour forward in Norway on 29.03.2020 minus 25 hours...
    d = new ServiceDate(2020, 3, 30);
    assertEquals(
      "2020-03-29T00:00+01:00[Europe/Oslo]",
      d.toZonedDateTime(zone, -23 * 3600).toString()
    );
  }
