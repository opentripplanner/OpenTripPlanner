...
class StopAreaMapperTest {
  ...
  @Test
  void map() {
    var stopMapper = new StopMapper(new TranslationHelper(), (ignored) -> null);
    var locationMapper = new LocationMapper();
    var mapper = new StopAreaMapper(stopMapper, locationMapper);
    ...
    var stopIds = areaStop.getLocations().stream().map(l -> l.getId().toString()).collect(Collectors.toSet());
