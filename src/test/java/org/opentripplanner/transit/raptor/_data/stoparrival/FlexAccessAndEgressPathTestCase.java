...
public class FlexAccessAndEgressPathTestCase implements RaptorTestConstants {
  ...
  public static final RaptorSlackProvider SLACK_PROVIDER = RaptorSlackProvider.defaultSlackProvider(
    TRANSFER_SLACK,
    BOARD_SLACK,
    ALIGHT_SLACK
  );
  ...
  public static final TestTransfer ACCESS_W_OPENING_HOURS = ACCESS.openingHours(
    ACCESS_OPEN,
    ACCESS_CLOSE
  );
  ...
  public static final TestTransfer EGRESS_W_OPENING_HOURS = EGRESS.openingHours(
    EGRESS_OPENING,
    EGRESS_CLOSING
  );

  public static final int EGRESS_COST_W_1M_SLACK =
    EGRESS_COST + toRaptorCost(TRANSFER_COST_SEC) + COST_CALCULATOR.waitCost(TRANSFER_SLACK);
  public static final int EGRESS_COST_W_7M45S_SLACK =
    EGRESS_COST_W_1M_SLACK + COST_CALCULATOR.waitCost(durationInSeconds("6m45s"));
  public static final int EGRESS_COST_W_9M45S_SLACK =
    EGRESS_COST_W_1M_SLACK + COST_CALCULATOR.waitCost(durationInSeconds("8m45s"));
  ...
  private static DestinationArrival<TestTripSchedule> flexForwardSearch(
    ...
    int additionalCost =
      egressPath.generalizedCost() + toRaptorCost(waitTime * WAIT_RELUCTANCE + TRANSFER_COST_SEC);
