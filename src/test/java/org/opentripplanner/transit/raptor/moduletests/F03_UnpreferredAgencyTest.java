 ...
 * by agency penalty.
 */
public class F03_UnpreferredAgencyTest implements RaptorTestConstants {

  private static final String EXPECTED =
    "Walk 30s ~ A ~ BUS %s 0:01 0:02:40 ~ B ~ Walk 20s " + "[0:00:30 0:03 2m30s 0tx $%d]";
  private static final FeedScopedId AGENCY_ID_1 = TransitModelForTest.id("1");
  private static final FeedScopedId AGENCY_ID_2 = TransitModelForTest.id("2");
  ...
  public void setup() {
    ...
    data.withRoute(
      route(pattern("R1", STOP_A, STOP_B))
        .withTimetable(schedule("00:01, 00:02:40").agencyId(AGENCY_ID_1))
    );
    data.withRoute(
      route(pattern("R2", STOP_A, STOP_B))
        .withTimetable(schedule("00:01, 00:02:40").agencyId(AGENCY_ID_2))
    );
  ...
  private void unpreferAgency(FeedScopedId routeId) {
    data.mcCostParamsBuilder().unpreferredAgencies(Set.of(routeId));
