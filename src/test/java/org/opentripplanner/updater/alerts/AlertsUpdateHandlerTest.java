...
public class AlertsUpdateHandlerTest {
  ...
  public void testWithoutUrlTranslations() {
    GtfsRealtime.Alert alert = Alert.newBuilder()
            .addInformedEntity(GtfsRealtime.EntitySelector.newBuilder().setAgencyId("1"))
            .setUrl(GtfsRealtime.TranslatedString.newBuilder().addTranslation(0, Translation.newBuilder().setText("https://www.opentripplanner.org/").build()).build())
            .build();
    TransitAlert transitAlert = processOneAlert(alert);
  ...
  public void testWithUrlTranslations() {
    GtfsRealtime.Alert alert = Alert.newBuilder()
            .addInformedEntity(GtfsRealtime.EntitySelector.newBuilder().setAgencyId("1"))
            .setUrl(GtfsRealtime.TranslatedString.newBuilder().addTranslation(0, Translation.newBuilder().setText("https://www.opentripplanner.org/").setLanguage("en").build()).addTranslation(0, Translation.newBuilder().setText("https://www.opentripplanner.org/fr").setLanguage("fr").build()).build())
            .build();
    TransitAlert transitAlert = processOneAlert(alert);

    List<Entry<String,String>> translations = ((TranslatedString) transitAlert.alertUrl).getTranslations();
  ...
  public void testWithoutHeaderTranslations() {
    GtfsRealtime.Alert alert = Alert.newBuilder()
            .addInformedEntity(GtfsRealtime.EntitySelector.newBuilder().setAgencyId("1"))
            .setHeaderText(GtfsRealtime.TranslatedString.newBuilder().addTranslation(0, Translation.newBuilder().setText("Title").build()).build())
            .build();
    TransitAlert transitAlert = processOneAlert(alert);
  ...
  public void testWithHeaderTranslations() {
    GtfsRealtime.Alert alert = Alert.newBuilder()
            .addInformedEntity(GtfsRealtime.EntitySelector.newBuilder().setAgencyId("1"))
            .setHeaderText(GtfsRealtime.TranslatedString.newBuilder().addTranslation(0, Translation.newBuilder().setText("Title").setLanguage("en").build()).addTranslation(0, Translation.newBuilder().setText("Titre").setLanguage("fr").build()).build())
            .build();
    TransitAlert transitAlert = processOneAlert(alert);

    List<Entry<String,String>> translations = ((TranslatedString) transitAlert.alertHeaderText).getTranslations();
    assertEquals(2, translations.size());
    assertEquals("en", translations.get(0).getKey());
  ...
  public void testWithoutDescriptionTranslations() {
    GtfsRealtime.Alert alert = Alert.newBuilder()
            .addInformedEntity(GtfsRealtime.EntitySelector.newBuilder().setAgencyId("1"))
            .setDescriptionText(GtfsRealtime.TranslatedString.newBuilder().addTranslation(0, Translation.newBuilder().setText("Description").build()).build())
            .build();
    TransitAlert transitAlert = processOneAlert(alert);
  ...
  public void testWithDescriptionTranslations() {
    GtfsRealtime.Alert alert = Alert.newBuilder()
            .addInformedEntity(GtfsRealtime.EntitySelector.newBuilder().setAgencyId("1"))
            .setDescriptionText(GtfsRealtime.TranslatedString.newBuilder().addTranslation(0, Translation.newBuilder().setText("Description").setLanguage("en").build()).addTranslation(0, Translation.newBuilder().setText("La description").setLanguage("fr").build()).build())
            .build();
    TransitAlert transitAlert = processOneAlert(alert);

    List<Entry<String,String>> translations = ((TranslatedString) transitAlert.alertDescriptionText).getTranslations();
    assertEquals(2, translations.size());
    assertEquals("en", translations.get(0).getKey());
  ...
  public void testAgencySelector() {
    ...
    long agencySelectorCount = transitAlert.getEntities().stream().filter(entitySelector -> entitySelector instanceof EntitySelector.Agency).count();
  ...
  public void testRouteSelector() {
    ...
    long totalSelectorCount = transitAlert.getEntities().size();
    assertEquals(1l, totalSelectorCount);
    long routeSelectorCount = transitAlert.getEntities().stream().filter(entitySelector -> entitySelector instanceof EntitySelector.Route).count();
  ...
  public void testTripSelectorWithTripId() {
    ...
    long totalSelectorCount = transitAlert.getEntities().size();
    assertEquals(1l, totalSelectorCount);
    long tripSelectorCount = transitAlert.getEntities().stream().filter(entitySelector -> entitySelector instanceof EntitySelector.Trip).count();
  ...
  public void testStopSelector() {
    ...
    long totalSelectorCount = transitAlert.getEntities().size();
    assertEquals(1l, totalSelectorCount);
    long stopSelectorCount = transitAlert.getEntities().stream().filter(entitySelector -> entitySelector instanceof EntitySelector.Stop).count();
  ...
  public void testStopAndRouteSelector() {
    ...
    long totalSelectorCount = transitAlert.getEntities().size();
    assertEquals(1l, totalSelectorCount);
    long stopAndRouteSelectorCount = transitAlert.getEntities().stream().filter(entitySelector -> entitySelector instanceof EntitySelector.StopAndRoute).count();
  ...
  public void testStopAndTripSelector() {
    GtfsRealtime.Alert alert = Alert.newBuilder()
            .addInformedEntity(0, GtfsRealtime.EntitySelector.newBuilder().setStopId("1").setTrip(
                    TripDescriptor.newBuilder().setTripId("1").build()))
            .build();
    TransitAlert transitAlert = processOneAlert(alert);
    long totalSelectorCount = transitAlert.getEntities().size();
    assertEquals(1l, totalSelectorCount);
    long stopAndTripSelectorCount = transitAlert.getEntities().stream().filter(entitySelector -> entitySelector instanceof EntitySelector.StopAndTrip).count();
