...
class EgressPathsTest {
  ...
  void byStopStandard() {
    ...
    assertEquals("[Walk 1m ~ 2]", byStop.get(STOP_A).toString());
    assertEquals("[Flex 1m 1x ~ 3]", byStop.get(STOP_B).toString());
    assertEquals("[Flex 1m 1x ~ 4]", byStop.get(STOP_C).toString());
    assertEquals("[Flex 1m 2x ~ 5]", byStop.get(STOP_D).toString());
    assertEquals(
      "[Walk 2m ~ 6, Walk 1m Open(10:00 11:45) ~ 6, Walk 1m Open(11:30 12:30) ~ 6]",
  ...
  void listAll() {
    assertEquals(
      ...
      Flex 1m 1x ~ 4
      Flex 1m 2x ~ 5
      Walk 1m Open(10:00 11:45) ~ 6
      Walk 1m Open(11:30 12:30) ~ 6
      Walk 1m ~ 2
      Walk 2m ~ 6
      """.strip(),
