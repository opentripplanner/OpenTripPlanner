...
public class CsvFileIO {
    ...
    public List<TestCase> readTestCasesFromFile() throws IOException {
        Map<String, org.opentripplanner.transit.raptor.speed_test.test.TestCaseResults> expectedResults = readExpectedResultsFromFile();
    ...
    public void writeResultsToFile(List<TestCase> testCases) {
        ...
        try (PrintWriter out = new PrintWriter(expectedResultsOutputFile, CHARSET_UTF_8.name())) {
            ...
            for (TestCase tc : testCases) {
                for (org.opentripplanner.transit.raptor.speed_test.test.Result result : tc.actualResults()) {
                    out.print(tc.id);
    ...
    private Map<String, org.opentripplanner.transit.raptor.speed_test.test.TestCaseResults> readExpectedResultsFromFile() throws IOException {
        if (!expectedResultsFile.exists()) {
            return Collections.emptyMap();
        }

        Map<String, org.opentripplanner.transit.raptor.speed_test.test.TestCaseResults> results = new HashMap<>();
        ...
        while (csvReader.readRecord()) {
            ...
            org.opentripplanner.transit.raptor.speed_test.test.Result expRes = readExpectedResult(csvReader);
            results.computeIfAbsent(expRes.testCaseId, org.opentripplanner.transit.raptor.speed_test.test.TestCaseResults::new).addExpectedResult(expRes);
        }
        return results;
    }

    private org.opentripplanner.transit.raptor.speed_test.test.Result readExpectedResult(CsvReader csvReader) throws IOException {
        try {
            return new org.opentripplanner.transit.raptor.speed_test.test.Result(
                    csvReader.get("tcId"),
