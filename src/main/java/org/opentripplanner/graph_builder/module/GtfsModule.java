    ...
    private GtfsMutableRelationalDao loadBundle(GtfsBundle gtfsBundle)
        ...
        if (bundleFilenamesWithAutoGeneratedFeedId.contains(bundleFilename)) {
            throw new RuntimeException(
                String.format(
                    "There are at least 2 bundles with the name `%s` that lack a feed_id value in a record in the feed_info.txt table. This will result in non-deterministic Feed ID generation!",
                    bundleFilename
                )
            );
        }
        ...
        // to determine whether an agency ID was auto-generated. According to the GTFS spec, if there are multiple
