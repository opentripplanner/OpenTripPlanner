...
class MissingElevationHandler {
    ...
    void run() {
        ...
        elevations.keySet().forEach(vertex -> {
            vertex.getIncomingStreetEdges().forEach(edge -> assignElevationToEdgeIfPossible(elevations, edge));
            vertex.getOutgoingStreetEdges().forEach(edge -> assignElevationToEdgeIfPossible(elevations, edge));
        });
    }
                ...
                (vertex, elevation) -> {
                    vertex.getIncoming().forEach(edge -> {
                        if (edge.getDistanceMeters() < maxElevationPropagationMeters) {
                            pq.insert(new ElevationRepairState(
                                    vertex, elevation, edge.getFromVertex(),
                    ...
                    vertex.getOutgoing().forEach(edge -> {
                        if (edge.getDistanceMeters() < maxElevationPropagationMeters) {
                            pq.insert(new ElevationRepairState(
                                    vertex, elevation, edge.getToVertex(),
                                    edge.getDistanceMeters()
                            ), edge.getDistanceMeters());
                        }
                    });
    ...
    private void assignElevationToEdgeIfPossible(Map<Vertex, Double> elevations, StreetEdge edge) {
        ...
        if (fromElevation == null || toElevation == null) {
            if (!edge.isElevationFlattened() && !edge.isSlopeOverride()) {
                issueStore.add(new ElevationProfileFailure(edge,
                        "Failed to propagate elevation data"
                ));
            }
            return;
        }
