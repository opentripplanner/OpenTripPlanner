...
import static org.opentripplanner.raptor.api.request.RaptorProfile.MIN_TRAVEL_DURATION_AND_COST_BEST_TIME;
  ...
  private void createHeuristicSearchIfNotExist(RaptorRequest<T> request) {
    if (search == null) {
      var builder = request
        ...
        .clearOptimizations()
        .profile(MIN_TRAVEL_DURATION_AND_COST_BEST_TIME)
        .searchDirection(direction);
