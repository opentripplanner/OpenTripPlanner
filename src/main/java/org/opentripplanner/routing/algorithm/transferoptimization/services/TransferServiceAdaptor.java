...
/**
 * This a just a adaptor to look up guaranteed transfers. The adaptor hide the
 * {@link TransferService} specific API and function as a bridge to the
 * {@code transferoptimization} model. The best solution would be to use the same
 * mechanism in Raptor and here, but that requires the main transit model to be refactored.
 * <p>
 * The adaptor make it easy to test the {@link TransferGenerator} by mocking.
 *
