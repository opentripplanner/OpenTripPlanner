...
class JavaImmutableMapSerializer extends Serializer<Map<?, ?>> {
    ...
    public void write(Kryo kryo, Output output, Map<?, ?> map) {
        ImmSerMapEntry[] entries = map.entrySet().stream()
    ...
    private static class ImmSerMap implements Serializable  {
        ...
        private ImmSerMap(ImmSerMapEntry[] array) { this.array = array; }
        private Map.Entry[] getEntries() { return array; }
    }
    ...
    private static class ImmSerMapEntry<K, V> implements Map.Entry<K, V>, Serializable {
        ...
        @Override public K getKey() { return (K) entry[0]; }
        @Override public V getValue() { return (V) entry[1]; }
