...
public class GraphPathFinder {
    ...
    public List<GraphPath> graphPathFinderEntryPoint (RoutingRequest request) {
        ...
        // post-process paths to remove very short TNC trips
        if (request.useTransportationNetworkCompany) {
            // iterate through each path and rewrite state of last mile if it contains a very short TNC trip
            int pathIdx = 0;
            for (GraphPath path : paths) {
                if (request.arriveBy) {
                    // TODO: implement looking at beginning leg of trip
                    State curState = path.states.get(0);
                    State lastState = null;
                    if (curState.isUsingHailedCar()) {
                        // using a hailed car from the start.
                        // Make sure length of trip is greater than minimum required distance
                        int curStateIdx = 1;
                        while(curStateIdx < path.states.size()) {
                            curState = path.states.get(curStateIdx);
                            if (!curState.isUsingHailedCar()) {
                                // reached end of initial ride hail
                                if (lastState.transportationNetworkCompanyDriveDistance <
                                    request.minimumTransportationNetworkCompanyDistance) {
                                    // trip not long enough, replan with walking
                                    RoutingRequest lastMile = request.clone();
                                    lastMile.setMode(TraverseMode.WALK);
                                    lastMile.useTransportationNetworkCompany = false;
                                    // set start time to be time in last observed state
                                    Date lastMileArriveByTime = new Date();
                                    lastMileArriveByTime.setTime(curState.getTimeInMillis());
                                    lastMile.setDateTime(lastMileArriveByTime);

                                    // remove all states and edges up until transit
                                    // keep original path states and edges in case walking can't be found
                                    LinkedList<State> removedStates = new LinkedList<>();
                                    LinkedList<Edge> removedEdges = new LinkedList<>();

                                    // remove beginning state to avoid Null Pointer Exception since
                                    // first state does not have a back edge
                                    removedStates.add(path.states.removeFirst());
                                    while (path.states.size() > 0) {
                                        curState = path.states.getFirst();
                                        if (
                                            curState.stateData.getBackMode().equals(TraverseMode.WALK) ||
                                            curState.stateData.getBackMode().equals(TraverseMode.CAR)
                                            ) {
                                            removedStates.add(path.states.removeFirst());
                                        } else {
                                            break;
                                    ...
                                    while (path.edges.size() > 0) {
                                        Edge curEdge = path.edges.getFirst();
                                        if (curEdge.getFromVertex().equals(curState.getVertex())) {
                                            break;
                                        }
                                        removedEdges.add(path.edges.removeFirst());
                                    }

                                    // update start and end vertices
                                    lastMile.setRoutingContext(
                                        router.graph,
                                        request.rctx.fromVertex,
                                        curState.getVertex());
                                    List<GraphPath> walkPaths = getPaths(lastMile);

                                    if (walkPaths.size() == 0) {
                                        // no walking paths found for some reason!  Add back original states and edges
                                        while (removedStates.size() > 0) {
                                            path.states.addFirst(removedStates.removeFirst());
                                        }
                                        while (removedEdges.size() > 0) {
                                            path.edges.addFirst(removedEdges.removeFirst());
                                        }
                                    } else {
                                        GraphPath walkPath = walkPaths.get(0);
                                        State firstWalkState = walkPath.states.get(0);

                                        // splice remaining states into new walking path

                                        // remove last walk path as it's a termination step
                                        walkPath.states.removeLast();

                                        State curBeginningState = path.states.getFirst();
                                        curBeginningState.backState = walkPath.states.getLast();

                                        // add walking states to begging of rest of trip
                                        path.states.addAll(0, walkPath.states);

                                        // add all edges to output
                                        path.edges.addAll(0, walkPath.edges);
                                    }
                                }
                                break;
                            }
                    ...
                    State lastState = path.states.get(path.states.size() - 1);
                    if (lastState.isUsingHailedCar() &&
                        lastState.transportationNetworkCompanyDriveDistance <
                            request.minimumTransportationNetworkCompanyDistance
                        ) {
                        // not a far enough drive, redo search between tnc start vertex and end vertex
                        // replace states and edges with walk-only search result
                        RoutingRequest lastMile = request.clone();
                        lastMile.setMode(TraverseMode.WALK);
                        lastMile.useTransportationNetworkCompany = false;
                        Vertex lastLegVertex = null;
                        State curState = null;
                        ...
                        // trim states and edges until we've reached the deboarding of transit
                        while (path.states.size() > 0) {
                            curState = path.states.getLast();
                            if (
                                curState.stateData.getBackMode() == null ||
                                curState.stateData.getBackMode().equals(TraverseMode.WALK) ||
                                curState.stateData.getBackMode().equals(TraverseMode.CAR)
                                ) {
                                removedStates.add(path.states.removeLast());
                                lastLegVertex = curState.getVertex();
                            } else {
                                break;
                            }
                        }
                        ...
                        while (path.edges.size() > 0) {
                            Edge curEdge = path.edges.peekLast();
                            if (curEdge.getFromVertex().equals(lastLegVertex)) {
                                break;
                            }
                            removedEdges.add(path.edges.removeLast());
                        }

                        // set start time to be time in last observed state
                        Date lastMileStart = new Date();
                        lastMileStart.setTime(curState.getTimeInMillis());
                        lastMile.setDateTime(lastMileStart);

                        // update start and end vertices
                        lastMile.setRoutingContext(router.graph, lastLegVertex, request.rctx.toVertex);
                        List<GraphPath> walkPaths = getPaths(lastMile);
                        if (walkPaths.size() == 0) {
                            // no walking paths found for some reason!  Add back original states and edges
                            while (removedStates.size() > 0) {
                                path.states.add(removedStates.removeLast());
                            }
                            while (removedEdges.size() > 0) {
                                path.edges.add(removedEdges.removeLast());
                            }
                        } else {
                            GraphPath walkPath = walkPaths.get(0);
                            State firstWalkState = walkPath.states.get(0);

                            // splice states into old path
                            State curEndState = path.states.getLast();
                            firstWalkState.backState = curEndState;
                            // remove first walk state and then add the rest
                            walkPath.states.removeFirst();
                            path.states.addAll(walkPath.states);

                            // add all edges to output
                            path.edges.addAll(walkPath.edges);
                        }
                    }
                }
                pathIdx++;
