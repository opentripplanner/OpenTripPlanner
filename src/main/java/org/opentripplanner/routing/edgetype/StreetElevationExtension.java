...
public class StreetElevationExtension implements Serializable {
    ...
    private static StreetElevationExtension calculateForEdge(
        ...
        float effectiveBicycleSafetyFactor = (float) (
                streetEdge.getBicycleSafetyFactor() * costs.lengthMultiplier
                        + costs.slopeSafetyCost / streetEdge.getDistanceMeters()
        );

        if (Double.isInfinite(effectiveBicycleSafetyFactor) || Double.isNaN(
