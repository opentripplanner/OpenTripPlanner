  ...
  public State traverse(State s0) {
    ...
    if (s0.getNonTransitMode() == TraverseMode.BICYCLE) {
      if (canTraverse(TraverseMode.BICYCLE)) {
        editor = doTraverse(s0, options, preferences, TraverseMode.BICYCLE, false);
      } else if (canTraverse(TraverseMode.WALK)) {
        editor = doTraverse(s0, options, preferences, TraverseMode.WALK, true);
      } else {
        return null;
      }
    } else if (canTraverse(s0.getNonTransitMode())) {
      editor = doTraverse(s0, options, preferences, s0.getNonTransitMode(), false);
    } else {
      editor = null;
    }
    ...
    if (canPickupAndDrive(s0)) {
      StateEditor inCar = doTraverse(s0, options, preferences, TraverseMode.CAR, false);
      if (inCar != null) {
    ...
    if (canDropOffAfterDriving(s0) && !getPermission().allows(TraverseMode.CAR)) {
      StateEditor dropOff = doTraverse(s0, options, preferences, TraverseMode.WALK, false);
      if (dropOff != null) {
