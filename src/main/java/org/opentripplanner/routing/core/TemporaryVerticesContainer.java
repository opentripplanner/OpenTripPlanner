...
public class TemporaryVerticesContainer implements AutoCloseable {
  ...
  public TemporaryVerticesContainer(Graph graph, RoutingRequestAndPreferences opt) {
    this.tempEdges = new HashSet<>();

    this.graph = graph;
    StreetVertexIndex index = this.graph.getStreetIndex();
    this.opt = opt;
    fromVertices = index.getVerticesForLocation(opt.request().from(), opt, false, tempEdges);
    toVertices = index.getVerticesForLocation(opt.request().to(), opt, true, tempEdges);

    checkIfVerticesFound(opt.request().arriveBy());
  ...
  private void checkIfVerticesFound(boolean arriveBy) {
    ...
    if (opt.request().from().isSpecified() && isDisconnected(from, true)) {
      routingErrors.add(
        new RoutingError(
          getRoutingErrorCodeForDisconnected(opt.request().from()),
    ...
    if (opt.request().to().isSpecified() && isDisconnected(to, false)) {
      routingErrors.add(
        new RoutingError(
          getRoutingErrorCodeForDisconnected(opt.request().to()),
          InputField.TO_PLACE
        )
      );
    }
  ...
  private boolean isDisconnected(Set<Vertex> vertices, boolean isFrom) {
    ...
    Predicate<Vertex> isNotConnected = (isFrom == opt.request().arriveBy())
      ? hasNoIncoming
      : hasNoOutgoing;
