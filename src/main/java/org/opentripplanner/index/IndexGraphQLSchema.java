...
public class IndexGraphQLSchema {
    ...
    public IndexGraphQLSchema(GraphIndex index) {
        ...
        GraphQLInputObjectType triangleInputType = GraphQLInputObjectType.newInputObject()
            .name("InputTriangle")
            .description("Relative importances of optimization factors. Only effective for bicycling legs. Invariant: `timeFactor + slopeFactor + safetyFactor == 1`")
            .field(GraphQLInputObjectField.newInputObjectField()
                .name("safetyFactor")
        ...
        bikeRentalStationType = GraphQLObjectType.newObject()
            ...
            .field(GraphQLFieldDefinition.newFieldDefinition()
		        ...
		        .description("Number of free spaces currently available on the rental station. The total capacity of this bike rental station is the sum of fields `bikesAvailable` and `spacesAvailable`. Note that this value being 0 does not necessarily indicate that bikes cannot be returned to this station, as it might be possible to leave the bike in the vicinity of the rental station, even if the bike racks don't have any spaces available (see field `allowDropoff`).")
                .type(Scalars.GraphQLInt)
