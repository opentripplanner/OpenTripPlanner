...
public class IndexAPI {
    ...
    public Response getFeeds() {
        return Response.status(Status.OK).entity(index.getAgenciesForFeedId().keySet()).build();
    }

    @GET
    @Path("/feeds/{feedId}")
    public Response getFeedInfo(@PathParam("feedId") String feedId) {
        FeedInfo feedInfo = index.getFeedInfoForId().get(feedId);
        if (feedInfo == null) {
   ...
   public Response getAgencies (@PathParam("feedId") String feedId) {
       return Response.status(Status.OK).entity(
               index.getAgenciesForFeedId().getOrDefault(feedId, new HashMap<>()).values()).build();
   }
   ...
   public Response getAgency (@PathParam("feedId") String feedId, @PathParam("agencyId") String agencyId) {
       for (Agency agency : index.getAgenciesForFeedId().get(feedId).values()) {
    ...
    public Response getAgencyRoutes (@PathParam("feedId") String feedId, @PathParam("agencyId") String agencyId) {
        Collection<Route> routes = index.getRouteForId().values();
        Agency agency = index.getAgenciesForFeedId().get(feedId).get(agencyId);
   ...
   public Response getRoutesForStop (@PathParam("stopId") String stopId) {
       Stop stop = index.getStopForId().get(GtfsLibrary.convertIdFromString(stopId));
       if (stop == null) return Response.status(Status.NOT_FOUND).entity(MSG_404).build();
       Set<Route> routes = Sets.newHashSet();
       for (TripPattern pattern : index.getPatternsForStop().get(stop)) {
   ...
   public Response getPatternsForStop (@PathParam("stopId") String stopIdString) {
       ...
       if (stop == null) return Response.status(Status.NOT_FOUND).entity(MSG_404).build();
       Collection<TripPattern> patterns = index.getPatternsForStop().get(stop);
    ...
    public Response getStoptimesForStop (@PathParam("stopId") String stopIdString,
        ...
        Stop stop = index.getStopForId().get(GtfsLibrary.convertIdFromString(stopIdString));
        if (stop == null) return Response.status(Status.NOT_FOUND).entity(MSG_404).build();
    ...
    public Response getTransfers(@PathParam("stopId") String stopIdString) {
        ...
        if (stop != null) {
            ...
            TransitStopVertex v = index.getStopVertexForStop().get(stop);
   ...
   public Response getPatternsForRoute (@PathParam("routeId") String routeIdString) {
       ...
       Route route = index.getRouteForId().get(routeId);
       if (route != null) {
           Collection<TripPattern> patterns = index.getPatternsForRoute().get(route);
           return Response.status(Status.OK).entity(PatternShort.list(patterns)).build();
       } else { 
           return Response.status(Status.NOT_FOUND).entity(MSG_404).build();
       }
   ...
   public Response getPatterns () {
       Collection<TripPattern> patterns = index.getTripPatterns();
       return Response.status(Status.OK).entity(PatternShort.list(patterns)).build();
   }
   ...
   public Response getPattern (@PathParam("patternId") String patternIdString) {
       TripPattern pattern = index.getTripPatternForId(patternIdString);
       if (pattern != null) {
