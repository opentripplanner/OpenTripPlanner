...
public class KmlBikeParkDataSource implements DataSource<VehicleParking> {
  ...
  public KmlBikeParkDataSource(KmlUpdaterParameters parameters) {
    ...
    xmlDownloader.setPath(
      "//*[local-name()='kml']/*[local-name()='Document']/*[local-name()='Placemark']|//*[local-name()='kml']/*[local-name()='Document']/*[local-name()='Folder']/*[local-name()='Placemark']"
    );
    xmlDownloader.setDataFactory(attributes -> {
      ...
      var id = String.format(ROOT, "%s[%.3f-%.3f]", name.replace(" ", "_"), x, y);
