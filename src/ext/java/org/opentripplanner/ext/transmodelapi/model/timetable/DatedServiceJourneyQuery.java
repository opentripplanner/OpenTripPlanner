    ...
    public static GraphQLFieldDefinition createQuery(
        ...
        return GraphQLFieldDefinition.newFieldDefinition()
                ...
                .argument(GraphQLArgument.newArgument()
                        ...
                        .type(new GraphQLList(new GraphQLNonNull(Scalars.GraphQLString))))
                .dataFetcher(environment -> {
                    Stream<TripOnServiceDate> stream = GqlUtil.getRoutingService(environment).getTripOnServiceDateById().values().stream();
                    ...
                    if (privateCodes != null && !privateCodes.isEmpty()) {
                        stream = stream.filter(tripOnServiceDate ->
                                privateCodes.contains(
                                        tripOnServiceDate.getTrip().getInternalPlanningCode()
                                )
                        );
                    }
                    ...
                    if (!authorities.isEmpty()) {
                        stream = stream.filter(tripOnServiceDate ->
                                authorities.contains(
                                        tripOnServiceDate.getTrip().getRoute().getAgency().getId()
                                )
                        );
                    }
