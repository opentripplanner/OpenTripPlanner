...
public record GeofencingBoundaryExtension(GeofencingZone zone, boolean entering) {
  /**
   * Whether this boundary is paired with {@code other} — same zone, opposite entering flag. A pair
   * on the two endpoints of an edge indicates the edge crosses the zone boundary.
   */
  ...
  /**
   * Whether any boundary in the list marks an entry into a no-traversal zone for the given network.
   */
  ...
  /**
   * Resolve zone transitions for a boundary-crossing edge. For each boundary on fromVertex, checks
   * if toVertex has a paired boundary (same zone, opposite entering flag). Paired boundaries
   * indicate the edge crosses a zone boundary, adding or removing the zone from the current set.
   * The arriveBy flag is XOR'd with the entering flag to handle reverse searches.
   *
