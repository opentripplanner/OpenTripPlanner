...
/**
 * The name, permission and safety factors a visibility edge borrows from the area(s) it crosses. A
 * multi-area group's sub-areas tile/overlap to cover the group, so a visibility edge routinely
 ...
 * crossing a {@code steps} tile cannot keep an adjacent square's flat walk cost or bike
 * permission). Wheelchair-accessibility is likewise merged with a logical AND, so any
 * non-accessible tile the edge crosses makes the whole edge non-accessible. The name is cosmetic
 * and is taken from the first crossed sub-area.
 */
record AreaEdgeProperties(
  ...
  /**
   * Folds the worst-case properties over every area in {@code crossed}, which must be non-empty
   * (see {@link PreparedAreaGroup#areasCrossedBy(org.locationtech.jts.geom.LineString)}).
   */
