 ...
 * <p>
 * An instance of this class is passed on to the algorithm to perform the boarding and contain
...
public interface RaptorBoardOrAlightEvent<T extends RaptorTripSchedule> {
  ...
  /**
   * For a regular boarding, return the earliest-board-time passed in to the trip search. For
   * boardings using a constrained transfer the trip search must calculate the earliest-board-time,
   * because it depends on the constraints.
   * <p>
   ...
   * constraints assisiated with the boarding the
   * {@link RaptorTransferConstraint#isRegularTransfer()} is {@code true}.
   */
  RaptorTransferConstraint transferConstraint();

  /**
   * This method return true if no result is found, but the algorithm may continue to search using
   * another way of boarding. The result is NOT empty if it is forbidden.
   */
  boolean empty();
   ...
   * <p>
   * Sometimes we need to override the search result and force an empty result. This method can
   * be used in none-performance critical parts of the code. The preferred way for
   * {@link RaptorConstrainedBoardingSearch}s is to clear its state and implement the fly-weight
   * pattern, avoiding creating new instances.
   */
