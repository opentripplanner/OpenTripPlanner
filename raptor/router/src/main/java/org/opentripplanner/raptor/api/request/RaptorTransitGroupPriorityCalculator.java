...
public interface RaptorTransitGroupPriorityCalculator {
  /**
   * Merge in the transit group id with an existing set. Note! Both the set and the group id type is
   * {@code int}.
   *
  ...
  /**
   * This is the dominance function to use for comparing transit-groups. It is critical that the
   * implementation is "static" so it can be inlined, since it is run in the innermost loop of
   * Raptor.
   */
