...
/// Pareto Dominance can have 4 values when comparing two values `x`, `y`. The 4 values are mutually
/// exclusive. Remember x and y can be vectors with one or more criteria.
public enum ParetoDominance {
  ...
  /// other. Both solutions are part of the Pareto optimal set (Pareto front). This happens when the
  /// `x` is better in one criteria, and `y` is better in another: `[1, 7, 3]` ∥ `[7, 1, 3]`.
  MUTUAL('∥'),
  /// No Dominate (Strictly equal) (`x` ≡ `y`). Neither `x` dominates `y` nor `y` dominates `x`, AND
  /// `x` and `y` are equal in all objective values: `[1, 7, 3]` ≡ `[1, 7, 3]`.
  NONE('≡');
   ...
   *
   * <p>
   * Accepted symbols are `≺`, `≻`, `∥`, and `≡`. If the input is not a single-character symbol,
