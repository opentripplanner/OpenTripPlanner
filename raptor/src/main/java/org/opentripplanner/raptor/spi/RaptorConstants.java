 ...
 * parameters/return values for functions and methods in the API and SPI. These are not meant to be
 * global OTP constants, so use them within Raptor and in interaction with Raptor only.
 * <p>
 * When choosing a value we avoid {@code -1}, {@link Integer#MIN_VALUE} and
 * {@link Integer#MIN_VALUE}. {@code -1} is often a legal value and a calculation error can often
 * <em>overflow</em> to {@code -1} - even for strictly positive types like
 * {@code number-of-transfers}. So we use big negative "random" numbers - with some distance apart
 * from each other. We do not use {@link Integer#MIN_VALUE}, because this could potentially lead to
 * overflow situations which would be very hard to debug. If you add -1 to {@code Integer.MIN_VALUE}
 * and you get a positive number - not an exception.
 */
public class RaptorConstants {
   ...
   * {@code generalized-cost}, {@code link min-travel-time} and {@code duration} inside Raptor. Most
   * of these types are "positive" numbers, but we want Raptor to be robust and work even for
   * negative values; Hence we choose a large negative value.
   */
  public static final int NOT_SET = -1_999_000_000;

  /**
   * When searching for a stop position or trip index we will use this to signal NOT FOUND. We
   * distinguish between this and {@link #NOT_SET}.
   */
   ...
   * this constant. It is used when a small value is considered better than a high value. Since all
   * real-world values are better than this, only one comparison is needed to check if a new
  ...
  /**
   * In cases where a large value is better than a low value we initialize with UNREACHED_LOW. For
   * example, this is the case when we search in reverse (time).
   * <p>
