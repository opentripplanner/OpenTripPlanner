...
public interface TransitCalculator<T extends RaptorTripSchedule> extends TimeCalculator {
   ...
   * reverse search return the next trips departure time at stop position with the boardSlack added.
   *
   ...
   * </ul>
   * It will add transit slack (egress leaves on-board) and then time-shift the egress.
   * <p>
   * It returns the calculated departure time or {@link RaptorConstants#TIME_NOT_SET} if boarding
   * is not possible.
   */
   ...
   * {@link #calculateEgressDepartureTime(int, RaptorAccessEgress, int)}, but instead of
   * time-shifting egress with opening hours the wait-time is ignored and only the egress duration
   * is added.
   */
