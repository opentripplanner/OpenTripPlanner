 ...
 * <p/>The {@link #bestTimePreviousRound(int)} return an estimate of the best time for the previous
 * round by using the overall best time (any round including the current round).
 * <p/>This class is used to calculate heuristic information like the best possible arrival times
 * and the minimum number for transfers. The results are an optimistic "guess", since we use the
...
public class BestTimesOnlyStopArrivalsState<T extends RaptorTripSchedule> implements
   ...
   * <p/>This is a simplification, *bestTimes* might get updated during the current round; Hence
   * leading to a new boarding at the alight stop in the same round. If we do not count rounds or
   * track paths, this is OK.
   * <p/>Because this rarely happens and heuristics does not need to be exact - it only need to be
   ...
   * <p/>The "arrival time" is calculated correctly.
   */
  @Override
