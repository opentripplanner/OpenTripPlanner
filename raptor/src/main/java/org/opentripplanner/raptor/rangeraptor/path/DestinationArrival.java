 ...
 * <p/>Compared with the ParetoSet of each stop we need two extra criteria:
 * <ul>
 *   <li>Number of transfers. The McRangeRaptor works in rounds, so there is no need to include rounds
 *       in the intermediate stop pareto sets. But to avoid that a later iteration delete an earlier
 *       result with less transfers, transfers need to be added as a criterion to the final destination.
 *
 * <li>Travel time duration - Range Raptor works in iteration. So when a later iteration makes it
 * into the destination set - it should not erase an earlier result unless it is faster. There is no
 * check on total travel duration for each stop, because it does not need to.
 * </ul>
 *
