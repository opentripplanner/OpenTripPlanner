 ...
 * route index and trip schedule index. A typical use-case for this is when you want to start a trip
 * on-board, meaning that one is already on the vehicle when the path starts. The returned paths
 * will start with a zero duration access and a boarding at the given stop.
 */
public interface RaptorStartOnBoardAccess extends RaptorAccessEgress {
  ...
  /**
   * The stop index corresponding to {@link #stopPositionInPattern()}. {@inheritDoc}
   */
  @Override
  int stop();
