...
public class GeometryUtilsTest {
    ...
    public final void testSplitGeometryAtFraction() {
        ...
        results[0][0] = result.getFirst();
        results[0][1] = result.getSecond();

        result = GeometryUtils.splitGeometryAtFraction(geometry, 0.125);
        results[1][0] = result.getFirst();
        results[1][1] = result.getSecond();

        result = GeometryUtils.splitGeometryAtFraction(geometry, 0.25);
        results[2][0] = result.getFirst();
        results[2][1] = result.getSecond();

        result = GeometryUtils.splitGeometryAtFraction(geometry, 0.375);
        results[3][0] = result.getFirst();
        results[3][1] = result.getSecond();

        result = GeometryUtils.splitGeometryAtFraction(geometry, 0.5);
        results[4][0] = result.getFirst();
        results[4][1] = result.getSecond();

        result = GeometryUtils.splitGeometryAtFraction(geometry, 0.625);
        results[5][0] = result.getFirst();
        results[5][1] = result.getSecond();

        result = GeometryUtils.splitGeometryAtFraction(geometry, 0.75);
        results[6][0] = result.getFirst();
        results[6][1] = result.getSecond();

        result = GeometryUtils.splitGeometryAtFraction(geometry, 0.875);
        results[7][0] = result.getFirst();
        results[7][1] = result.getSecond();

        result = GeometryUtils.splitGeometryAtFraction(geometry, 1);
        results[8][0] = result.getFirst();
        results[8][1] = result.getSecond();
    ...
    @Test
    public final void testBinarySearchCoordinates() {
        Coordinate[] coordinates = new Coordinate[5];

        // In order not to make the search too simple, the size of the array is not a power of 2.
        coordinates[0] = new Coordinate(0, 0);
        ...
        // All of these numbers were chosen specifically to avoid inaccurate floating-point results.
    ...
    @Test
    public final void testComputePartialLength() {
        Coordinate[] coordinates = new Coordinate[4];

        coordinates[0] = new Coordinate(0, 0);
        coordinates[1] = new Coordinate(0, 3);
        coordinates[2] = new Coordinate(4, 3);
        coordinates[3] = new Coordinate(0, 0);

        assertEquals(0.0, GeometryUtils.computePartialLength(coordinates, Integer.MIN_VALUE), 0.0);
        assertEquals(0.0, GeometryUtils.computePartialLength(coordinates, -1), 0.0);
        assertEquals(0.0, GeometryUtils.computePartialLength(coordinates, 0), 0.0);
        assertEquals(3.0, GeometryUtils.computePartialLength(coordinates, 1), 0.0);
        assertEquals(7.0, GeometryUtils.computePartialLength(coordinates, 2), 0.0);
        assertEquals(12.0, GeometryUtils.computePartialLength(coordinates, 3), 0.0);
        assertEquals(12.0, GeometryUtils.computePartialLength(coordinates, 4), 0.0);
        assertEquals(12.0, GeometryUtils.computePartialLength(coordinates, 5), 0.0);
        assertEquals(12.0, GeometryUtils.computePartialLength(coordinates, Integer.MAX_VALUE), 0.0);
    }
}
