...
public class TestOpenStreetMapGraphBuilder extends TestCase {
	...
	public void testGraphBuilder() throws Exception {
		...
		assertTrue("name of v2 must be like \"Kamiennog\u00F3rska\"; was "
				+ v2.getName(), v2.getName().contains("Kamiennog\u00F3rska"));
		assertTrue("name of v3 must be like \"Mariana Smoluchowskiego\"; was "
				+ v3.getName(), v3.getName()
		...
		for (Edge e : gg.getOutgoing(v2)) {
			if (e instanceof TurnEdge) {
				...
				if (tov == v3 || tov == v3back) {
					assertTrue("Turn cost wrong; expected ~90, was: "
							+ t.turnCost, Math.abs(t.turnCost - 90) < 3);
					v3EdgeExists = true;
				}
			}
		}
		...
		for (Edge e : gg.getOutgoing(v3)) {
			if (e instanceof TurnEdge) {
				...
				Vertex tov = t.getToVertex();
				if (tov == v4) {
					assertTrue("Turn cost too big: " + t.turnCost,
							t.turnCost < 5);
					v4EdgeExists = true;
				}
			}
	...
	public void testCreativeNaming() {
		...
		namer
				.setCreativeNamePattern("Highway with cycleway {cycleway} and access {access} and morx {morx}");
