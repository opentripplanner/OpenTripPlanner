 ...
 * <p>
 * This class provides methods to search for elements by expanding outward from a starting index
 * in both forward and backward directions simultaneously, finding the closest match to the starting
 * point.
 */
public class TwoWayLinearSearch {
   ...
   * simultaneously, testing indices at increasing offsets until a match is found or the bounds are
   * exceeded. This ensures the closest matching index to the starting point is found.
   * <p>
   * The search pattern is: startIndex, startIndex+1, startIndex-1, startIndex+2, startIndex-2,
   * ...
   *
   * @param startIndex the index to start searching from; if not within bounds, the method returns
   *                   empty. If the {@code startIndex} is out of bound, the start position is
   *                   adjusted to the closest position inside the search range.
   ...
   * @return an OptionalInt containing the nearest matching index, or empty if no match is found.
   */
