 ...
 * that only specific, power-of-two-numbered actions are executed while skipping others. This can be
 * useful in scenarios where some actions need to be performed less frequently, but still at
...
public class PowerOfTwoThrottle {
  ...
  /**
   * Executes the provided action only if the internal throttling condition evaluates to false. The
   * throttling mechanism ensures that only certain sequential actions are processed, based on the
   * internal counter's state. If the condition is met, the action is executed with the current
   * value of the counter as its argument.
   *
