...
[latest released version](http://dev.opentripplanner.org/jars/otp-0.17.0.jar)
or if you're feeling adventurous try the
[bleeding edge development code](http://dev.opentripplanner.org/jars/otp-0.18.0-SNAPSHOT.jar).
...
looks like `java -Xmx1G -jar otp-0.17.0.jar <options>`. The `-Xmx` parameter sets
...
For simplicity we'll skip saving this file and start up an OTP server immediately after the graph is built. The command to do so is:
    ...
    java -Xmx2G -jar otp-0.17.0.jar --build /home/username/otp --inMemory
