...
| `timeZone`                         | Time zone for the graph. This is used to store the timetables in the transit model, and to interpret times in incoming requests. This is required if the inout data contains agencies with multiple timezones. If empty it will use the time zone shared by all of the agencies. | string         | null                    | The value can be given either as a zone id, or an UTC offset.                             |
