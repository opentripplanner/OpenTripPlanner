...
The above query makes a request to the locally running server `http://localhost:8080/`, requesting the [planner resource](http://otp-docs.ibi-transit.com/api/resource_PlannerResource.html) `...otp/routers/default/plan`, and passes the following parameters:
...
Here we'll use of the [LIsochrone resource](http://otp-docs.ibi-transit.com/api/resource_LIsochrone.html) in the following query:
...
The result of the above query, if all goes well, is a geoJSON file with two multipolygons showing the area accessible within both one and one-half hour. Take a look at the [LIsochrone resource](http://otp-docs.ibi-transit.com/api/resource_LIsochrone.html) page for additional options. 
